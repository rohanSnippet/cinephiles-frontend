...
const SeatBookingLayout = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res) {
            Swal.fire({ title: "Deleted!", text: `${screen.sname} has been deleted.`, icon: "success", background: "#111", color: "white", showConfirmButton: false, timer: 1500 });
          }
          navigate("/owner/screen-details");
        } catch (error) {
          Swal.fire({ title: "Error!", text: "There was an error deleting the screen.", icon: "error", background: "#111", color: "white" });
        }
      }
    });
  };
  ...
  const handleSaveScreen = async () => {
    try {
      const response = await axiosSecure.put(`/screens/update/${screen.id}`, updatedScreen);
      if (response.status === 200) {
        Swal.fire({ position: "center", icon: "success", title: `${screen.sname} Layout Updated`, showConfirmButton: false, timer: 2000, background: "#111", color: "#fff" });
        navigate(`/owner/screen-details`);
      } else {
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#050505] font-poppins overflow-hidden w-full">
      ...
      <div className="w-full md:w-80 lg:w-96 shrink-0 bg-[#111] border-l border-white/5 flex flex-col shadow-2xl z-20">
        ...
        <div className="p-6 border-b border-white/5 bg-[#151515]">
          ...
          <p className="text-white/50 text-xs poppins-light">Updating: <span className="text-white poppins-medium">{screen.sname}</span></p>
        </div>
        ...
        {/* Final Save & Delete Area */}
        <div className="flex flex-col gap-3 p-6 bg-black/50 border-t border-white/5 mt-auto">
          <button
          ...
          >
            <MdSave size={20} /> Update Screen Layout
          </button>

          <button
            onClick={handleDeleteScreen}
            className="w-full py-3 rounded-xl bg-transparent border border-red-500/30 text-red-400 poppins-medium tracking-widest text-xs hover:bg-red-500/10 transition-all flex items-center justify-center gap-2"
          >
            <RiDeleteBin6Line size={16} /> Delete Screen
          </button>
        </div>
