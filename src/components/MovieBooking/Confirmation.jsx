...
const Confirmation = () => {
  ...
  <div className="max-w-xl mx-auto bg-slate-800 text-white shadow-lg rounded-lg overflow-hidden border">
    ...
    <div className="p-4 text-center border-b ">
      ...
      <p className="text-gray-100 text-sm mt-1">Booking ID <span className="font-bold">{data.bookingId}</span></p>
    </div>
    ...
    <div className="flex p-4 items-center gap-4 ">
      ...
      <div>
        ...
        <p className="text-gray-200 text-sm">{data.showTime} AM | Thu, 19 Jul, 2018</p>
        <p className="text-gray-200 text-sm">{data.theatre} ({data.location})</p>
        <p className="text-gray-200 text-sm">{data.theatreCity}, {data.theatreCity}</p>
      </div>
    </div>
    ...
    <div className="px-4 pb-4 flex justify-between items-center border-b">
      <div className='text-amber-300'>
        <p className="poppins-bold md:text-2xl text-xl">{seatNums.length}</p>
        <p className="md:text-md text-sm poppins-regular">Tickets</p>
      </div>
      <div>
        <p className="text-gray-300 text-lg poppins-medium">{data.tierName} - {data.seatIds}</p>
      </div>
      <div className="text-center">
        {/* <div className="text-green-600 font-bold border-2 border-green-600 px-2 py-1 rounded-md">Booking Confirmed</div> */}
        <img src={confirmed} alt="Booking Confirmed" srcset="" className='w-32'/>
      </div>
    </div>
    ...
    <div className="px-4 py-3 border-b flex justify-between items-center">
      ...
      <button className="bg-amber-500/80 text-white px-3 py-1 rounded text-sm poppins-light" onClick={navigate("/")}>Home Page</button>
    </div>
